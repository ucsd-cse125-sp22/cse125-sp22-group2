...
ObjPlayer::ObjPlayer(vector<PhysicalObject*>* objects,
                     unsigned int id,
                     glm::vec3 position,
                     glm::vec3 direction,
                     glm::vec3 up)
...
void ObjPlayer::step()
{
    ...
    // Increase score, currently set to not increase while
    // invincible/stunning/fixing makeup Eventually this will probably be based
    // on the amount of time left in the match, which would probably be passed
    // in as a parameter
    ...
    if (booth != -1 && boothTime)
    {
        glm::vec3 destination =
            (position + objects->at(booth)->position) / 2.0f;
...
void ObjPlayer::action(glm::vec3 dir)
{
    ...
    if (!stun || !boothTime)
    {
        ...
        // Generate a bounding box at our destination and check what we would
        // collide with
        BoundingBox bb = generateBoundingBox(destination, dir, this->up);

        // Make sure we don't drive out of the arena
        glm::vec3 arenaAdjustment =
            bounding::checkCollisionRadius(bb, MAP_CENTER, MAP_RADIUS);
        ...
        // Attempt to move out of a single collision (this is limited to one
        // attempt to avoid an infinite loop)
        bool adjusted = false;
        ...
        for (unsigned int i = 0; i < collisions.size(); i++)
        {
            ...
            if (obj->type == oPlayer)
            {
                ...
                ((ObjPlayer*)obj)
                    ->movePushed(dir, glm::dot(d, dir * this->speed));
            }
            ...
            if (obj->solid && !adjusted)
            {
                ...
                // "; ";
                glm::vec3 adjust =
                    bounding::checkCollisionAdjust(bb, obj->boundingBox);
                ...
                if (glm::length(adjust) < 0.5f)
                {
                    ...
                    BoundingBox temp = generateBoundingBox(destination + adjust,
                                                           dir, this->up);
            ...
            if (obj->type == oPlayer)
            {
                ...
                else if (((ObjPlayer*)obj)->hasCrown &&
                         !((ObjPlayer*)obj)->iframes)
                {
                    this->hasCrown = true;
                    this->iframes = 60;
...
bool ObjPlayer::objectPositionTagged(BoundingBox bb, int type, unsigned int id)
{
    ...
    for (unsigned int i = 0; i < objCount; i++)
    {
        ...
        if (this->objects->at(i)->type == type &&
            bounding::checkCollision(bb, this->objects->at(i)->boundingBox))
        {
            if (type == oTrail &&
                ((ObjTrail*)this->objects->at(i))->playerID != id)
            {
                return true;
            }
        }
...
void ObjPlayer::movePushed(glm::vec3 dir, float pushSpeed)
{
    ...
    BoundingBox bb =
        generateBoundingBox(destination, this->direction, this->up);
...
{
    BoundingBox bb =
        generateBoundingBox(this->position - glm::vec3(0.0f, 0.01f, 0.0f),
    ...
    if (checkPlaceFree(bb) && position.y > -12.0f)
    {
        ...
        glm::vec3 destination =
            this->position - glm::vec3(0.0f, this->gravity, 0.0f);
        ...
        for (unsigned int i = 0; i < collisions.size(); i++)
        {
            ...
            if (obj->solid && !adjusted)
            {
                ...
                if (glm::length(adjust) < 0.5f)
                {
                    ...
                    BoundingBox temp = generateBoundingBox(
                        destination + adjust, this->direction, this->up);
                    if (checkPlaceFree(temp))
                    {
                        // cout << "Adjusted\n";
                        destination += adjust;
